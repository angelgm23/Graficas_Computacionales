...
void Initialize()
{
	...
	//Leeemo el codgo fuente del vertex shader con la clase auxiliar InputFile
	ifile.Read("Default.vert");
	//Obtenemos los contenidos leido y los almacenamos en un string vertexSource
	std::string vertexSource = ifile.GetContents();
	// Hacemos un cast (conversion de tipo de dato) porque no le podemos mandar strings a OpenGL. Necesitamos mandarle const GLchar*.
	...
	//Creando el manager de los shaders. Almacenamos el id en la variable shaderProgram.
	shaderProgram = glCreateProgram();
	//Este manager va a administrar el vertex shader con identificador vertexShaderHandle
	glAttachShader(shaderProgram, vertexShaderHandle);
	//Este manager va a administrar el fragment shader con el identificarodr fragmentShaderHandle
	glAttachShader(shaderProgram, fragmentShaderHandle);
	//asociamos el indice del buffer (VBO) de posisicone con el nombre de la variable correspondiente al shader
